...
<head>
    ...
    <link rel="stylesheet" href="/Tov/viewTov_style.css" />
</head>
<body>
        ...
        if(!empty($query))   {
                        ...
                        { 
                            while ($row = mysqli_fetch_assoc($result)) 
                            {
                                    ...
                                    <li><form action="/Tov/DeleteTov.php" method="GET"> <input  type="hidden"  name="delete" value=<?php echo $row['Id_tov'] ?>>  <button class="submit1" type="submit" onclick="return confirm('Вы действительно хотите удалить?')">Удалить</button></form>
                                        <form action="/Tov/UpdateTov.php" method="GET"><input  type="hidden" class="submit" name="id" value=<?php echo $row['Id_tov']?>> <button class="submit1" type="submit">Изменить</button></form>
                                    </li>
                                </ul> 
                            <?php
                            }
